...
Revision ID: eaebd76208be
Revises: 0f3abb8488bf
Create Date: 2022-09-09 00:37:24.944036
...
revision = 'eaebd76208be'
down_revision = '0f3abb8488bf'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.create_foreign_key(op.f('fk_barcode_b_category_id_barcode_categories'), 'barcode', 'barcode_categories', ['b_category_id'], ['b_category_id'], ondelete='CASCADE')
    op.create_foreign_key(op.f('fk_barcode_company_id_barcode_companies'), 'barcode', 'barcode_companies', ['company_id'], ['company_id'], ondelete='CASCADE')
    op.drop_column('barcode', 'expiry_date')
    op.create_unique_constraint(op.f('uq_barcode_categories_b_category_id'), 'barcode_categories', ['b_category_id'])
    op.create_unique_constraint(op.f('uq_barcode_categories_b_category_name'), 'barcode_categories', ['b_category_name'])
    op.create_unique_constraint(op.f('uq_barcode_companies_company_id'), 'barcode_companies', ['company_id'])
    ...
    op.alter_column('recipe_situation', 'recipe_id',
               existing_type=sa.INTEGER(),
               nullable=True)
    op.create_foreign_key(op.f('fk_recipe_situation_recipe_id_recipes'), 'recipe_situation', 'recipes', ['recipe_id'], ['recipe_id'], ondelete='CASCADE')
    ...
    op.alter_column('recipes', 'recipe_desc',
               ...
               nullable=False)
    op.create_foreign_key(op.f('fk_situation_situation_id_recipe_situation'), 'situation', 'recipe_situation', ['situation_id'], ['situation_id'], ondelete='CASCADE')
...
def downgrade():
    ...
    op.drop_constraint(op.f('uq_barcode_companies_company_id'), 'barcode_companies', type_='unique')
    op.drop_constraint(op.f('uq_barcode_categories_b_category_name'), 'barcode_categories', type_='unique')
    op.drop_constraint(op.f('uq_barcode_categories_b_category_id'), 'barcode_categories', type_='unique')
